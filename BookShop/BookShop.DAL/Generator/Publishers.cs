...
{
	public partial class PublishersDAL
	{
...
VALUES (
	@Name
        ...
        {
            const string sql = "DELETE FROM [dbo].[Publishers] WHERE [Id] = @Id";
...
WHERE [Id] = @Id";
        ...
        public IEnumerable<Publishers> QueryList(int index, int size, object wheres, string orderField, bool isDesc = true)
        {
            ...
            var sql = SqlHelper.GenerateQuerySql("Publishers", new[] { "Id", "Name" }, index, size, whereBuilder.ToString(), orderField, isDesc);
            using (var reader = SqlHelper.ExecuteReader(sql, parameters.ToArray()))
            {
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        yield return SqlHelper.MapEntity<Publishers>(reader);
                    }
                }
            }
        }
        ...
        {
            const string sql = "SELECT TOP 1 [Id], [Name] FROM [dbo].[Publishers] WHERE [Id] = @Id";
            using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@Id", id)))
            {
                if (reader.HasRows)
                {
                    reader.Read();
                    return SqlHelper.MapEntity<Publishers>(reader);
                }
                else
                {
                    return null;
        ...
        public int QueryCount(object wheres)
        {
            ...
            var sql = SqlHelper.GenerateQuerySql("Publishers", new[] { "COUNT(1)" }, whereBuilder.ToString(), string.Empty);
