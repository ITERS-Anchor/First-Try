...
namespace BookShop.DAL
{
	...
	public class DepartmentDAL
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(BookShop.Model.Department model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Department(");
			strSql.Append("DepName,RoleID)");
		...
		/// 更新一条数据
		/// </summary>
		public void Update(BookShop.Model.Department model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public BookShop.Model.Department GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,DepName,RoleID from Department ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
			parameters[0].Value = ID;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,DepName,RoleID ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,DepName,RoleID ");
			strSql.Append(" FROM Department ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
